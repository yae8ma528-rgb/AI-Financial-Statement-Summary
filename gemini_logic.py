...
def send_message_with_fallback(client, content, prompt, system_instruction, previous_history=[]):
    """
    メッセージを送信し、429エラーが発生した場合はFallbackモデルで再試行する。
    
    Returns:
        tuple: (chat_session, response_text, model_name)
    """
    ...
    return active_chat, response_text, used_model

    return active_chat, response_text, used_model

def _clean_stream_generator(stream):
    """
...
def send_message_stream_with_fallback(client, content, prompt, system_instruction, previous_history=[]):
    ...
    for model_name in models_to_try:
        try:
            ...
            active_chat = chat
            # _clean_stream_generator には結合したジェネレータを渡す
            response_stream = _clean_stream_generator(reconstructed_stream())
            used_model = model_name
            break
